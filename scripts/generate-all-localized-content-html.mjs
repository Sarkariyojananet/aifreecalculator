...
function generateArticleHtml(entry, loc) {
  ...
  if (Array.isArray(entry.stepByStep) && entry.stepByStep.length > 0) {
    const stepsHtml = entry.stepByStep.map(s => `
    <li class="rounded-xl border border-slate-200/80 bg-slate-50/50 p-4 dark:border-slate-800 dark:bg-slate-900/40">
      <strong class="font-bold text-slate-900 dark:text-white block mb-1">${s.stepNumber ? s.stepNumber + '. ' : ''}${s.title}</strong>
      <span class="text-slate-600 dark:text-slate-300 text-sm leading-relaxed">${s.instruction}</span>
    </li>`).join('');
  ...
  if (Array.isArray(entry.variables) && entry.variables.length > 0) {
    const rowsHtml = entry.variables.map(v => `
      <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
        <td class="p-3 font-mono font-bold text-blue-600 dark:text-blue-400 border border-slate-200 dark:border-slate-800">${v.symbol}</td>
        <td class="p-3 font-semibold text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800">${v.label}</td>
        <td class="p-3 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-800 text-sm">${v.description}</td>
        <td class="p-3 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-800 text-xs font-mono">${v.unit || '-'}</td>
      </tr>`).join('');
  ...
  if (entry.workedExample && (entry.workedExample.title || entry.workedExample.scenario)) {
...
for (const file of files) {
  ...
  for (const loc of locales) {
    ...
    // Preserve existing rich custom article if present (length > 300)
    if (typeof entry.contentHtml === 'string' && entry.contentHtml.trim().length > 300) {
      preservedCount++;
      continue;
    }

    // Generate comprehensive article in this language
    const html = generateArticleHtml(entry, loc);
...
console.log('=== LOCALIZED CONTENT HTML GENERATION COMPLETE ===');
console.log('Files modified:', filesModified);
console.log('Articles generated:', generatedCount);
console.log('Articles preserved:', preservedCount);
